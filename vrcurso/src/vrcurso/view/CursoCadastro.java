...
public class CursoCadastro extends InternalFrame {
    ...
    public void salvar() throws Exception {
        ...
        oCurso.setLimiteVagas(Integer.parseInt(txtDuracao.getText()));
        oCurso.setCargaHoraria(Integer.parseInt(txtQtdeAlunos.getText()));
        oCurso.setDiaSemana(cboPeriodo.getId());
        oCurso.setIdProfessor(Integer.parseInt(txtCodProfessor.getText()));
        oCurso.setEmenta(txtEmenta.getText());

        oCurso = new DisciplinaService().salvar(oCurso);

        carregarDisciplina(oCurso.getId());
    ...
    public void carregarDisciplina(int i_id) throws Exception {
        ...
        oCurso = new DisciplinaService().consultar(oFiltro).get(0);
        txtCodigo.setText(Format.number(oCurso.getId(), 6));
        txtDescricao.setText(oCurso.getDescricao());
        txtDuracao.setText(String.valueOf(oCurso.getLimiteVagas()));
        txtQtdeAlunos.setText(String.valueOf(oCurso.getCargaHoraria()));
        cboPeriodo.setId(oCurso.getDiaSemana());
        txtCodProfessor.setText(String.valueOf(oCurso.getIdProfessor()));
        txtNomeProfessor.setText(oCurso.getProfessor());
        txtEmenta.setText(oCurso.getEmenta());
    }
    ...
    private void tblDadosMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblDadosMouseClicked
        try {
            if (evt.getClickCount() == 2) {
                if (isConsulta) {
                    txtCampoCod.setText(String.valueOf(vAluno.get(tblDados.convertRowIndexToModel(tblDados.getSelectedRow())).getId()));
                    txtCampoDesc.setText(String.valueOf(vAluno.get(tblDados.convertRowIndexToModel(tblDados.getSelectedRow())).getNome()));
                    dispose();
                } else {
                    editar();
                }
